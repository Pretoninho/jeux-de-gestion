...
- **Placement spatial** : chaque pack définit des `BuildingType` (recette + capacité + coût de construction) et une `grid: { width, height }`. `build()` (`src/engine/simulation.ts`) place une instance sur une case (vérifie case libre + dans les limites + argent suffisant) ; `EconomyState.placedBuildings` liste les instances posées (id, type, x, y). v1 volontairement modeste : grille fixe (pas de caméra/scroll), empreinte 1x1 partout, pas de démolition/déplacement — voir section "Concept de jeu actuel" pour le détail du phasage.
...
**Décision actée, implémentation reportée à une session future** : option 2 retenue — **composer un mini-bâtiment par case en empilant 2 sprites existants** (ex. mur + porte) plutôt que de chercher un nouveau pack ou de se contenter d'un sprite de ressource en dépannage. Nécessite d'étendre `renderTile()`/`ThemeAssets` pour accepter plusieurs sprites superposés par id (actuellement un seul sprite par id). Pas encore implémenté — à faire la prochaine fois que ce sujet revient.
...
- **Icônes de bâtiment composées de 2 sprites empilés** (option 2 actée, voir section "Assets — état actuel") — étendre `renderTile()`/`ThemeAssets` pour plusieurs sprites par id, puis choisir les paires mur+porte/etc. par type de bâtiment. Reporté à une session future sur demande explicite de l'utilisateur.
