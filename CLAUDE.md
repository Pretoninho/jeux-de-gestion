...
- **Différenciation par secteur** (confirmée explicitement) : chaque secteur a sa propre `taxSensitivity` et chaque catégorie a des poids différents par secteur — ex. pack `urban` : `logi` (taxSensitivity 1.15) plus sensible aux impôts que `comm` (0.85) ; catégorie "Écoles" profite plus au commerce (0.65) qu'à la logistique (0.35), "Santé" l'inverse.
